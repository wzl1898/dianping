-- ============================================================
-- 分库分表 DDL — tb_voucher_order 分片，其他表全量在 ds0
-- 版本: v2.0 (sharding preview)
-- ============================================================

-- ============================================================
-- ds0：完整数据源（所有 9 张表，含 tb_voucher_order）
-- ============================================================
CREATE DATABASE IF NOT EXISTS dianping_ds0 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE dianping_ds0;
...
INSERT IGNORE INTO tb_shop_type (`name`, `icon`, `sort`) VALUES
...
-- 6. 订单表（分片表，按 user_id 路由到 ds0 / ds1）
...
-- 初始化管理员
INSERT IGNORE INTO tb_admin (`username`, `password`, `nickname`) VALUES
('admin', '$2a$10$N.zmdr9k7uOCQb376NoUnuTJ8iAt6Z5EHsM8lE9lBOsl7iAt6Z5EH', '系统管理员');

-- ============================================================
-- ds1：分片数据源（只需 tb_voucher_order，其他表路由到 ds0）
-- ============================================================
CREATE DATABASE IF NOT EXISTS dianping_ds1 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
USE dianping_ds1;

-- 6. 订单表（分片表，与 ds0 结构完全一致）
CREATE TABLE IF NOT EXISTS tb_voucher_order (
    `id`          BIGINT   NOT NULL COMMENT '订单ID（分布式ID生成）',
    `user_id`     BIGINT   NOT NULL COMMENT '用户ID',
    `voucher_id`  BIGINT   NOT NULL COMMENT '优惠券ID',
